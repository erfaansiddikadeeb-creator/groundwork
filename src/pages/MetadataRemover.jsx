...
// Reads a TIFF-structured EXIF blob. Walks IFD0, then follows the Exif
// SubIFD pointer (tag 0x8769) so we pick up capture-time fields like
// DateTimeOriginal, FNumber, ISO, and FocalLength — not just the coarse
// IFD0 fields (Make/Model/Software/DateTime-modified).
function parseTiff(view, tiffStart) {
  ...
  function readIFD(ifdStart, sink) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (tag === 0x8769) exifOffset = tiffStart + get32(valueOffsetField);
      sink[tag] = value;
    }
  }

  readIFD(tiffStart + ifd0Offset, tags);
  if (exifOffset) {
    try { readIFD(exifOffset, tags); } catch (e) { /* ignore malformed SubIFD */ }
  }
  ...
  // Prefer the true capture time (Exif SubIFD) over the coarser
  // "file last modified" timestamp that lives in IFD0.
  if (tags[0x9003]) result["Date taken"] = tags[0x9003];
  else if (tags[0x0132]) result["File modified"] = tags[0x0132];
  if (tags[0x920a]) result["Focal length"] = tags[0x920a] + " mm";
  if (tags[0x829d]) result["Aperture"] = "f/" + tags[0x829d];
  if (tags[0x8827]) result["ISO"] = tags[0x8827];
...
async function loadAnyImage(file) {
  ...
  if (kind === "tiff") {
    ...
    // UTIF reads raw pixel data directly — it bypasses the browser's own
    // image decoder, so nothing has auto-rotated this yet. We still need
    // to apply the orientation tag by hand here.
  ...
  // Standard path: JPEG / PNG / WebP, decoded via a plain <img>.
  ...
  // IMPORTANT: browsers already auto-rotate a decoded <img> according to
  // its EXIF orientation tag (this has been the default rendering
  // behavior in Chrome/Firefox/Safari for years). img.naturalWidth /
  // naturalHeight already reflect that rotated orientation. Calling
  // orientCanvas() here as well would apply the same rotation a SECOND
  // time — which is exactly the "rotated 90° the wrong way" bug that
  // shows up on phone photos (phones almost always tag a non-1
  // orientation) but not on typical PC-authored images (which usually
  // carry no orientation tag, so there's nothing to double-apply).
  // So: draw the already-correctly-oriented image straight to canvas.
  const canvas = document.createElement("canvas");
  canvas.width = img.naturalWidth;
  canvas.height = img.naturalHeight;
  canvas.getContext("2d").drawImage(img, 0, 0);
...
export default function MetadataRemover() {
  ...
  // Track the most recent object URL we handed out for downloads, so we
  // can revoke it before minting a new one instead of leaking blob URLs.
  ...
  function reset() {
    revokeIfSet(beforeUrl);
    revokeIfSet(afterUrl);
    revokeIfSet(downloadUrlRef.current);
