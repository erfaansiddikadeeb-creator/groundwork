...
export default function ImageConverter() {
  ...
  // Growing the width/height past what fits at the box's current position
  // shifts the box back into bounds rather than silently capping the
  // number — so typing "1200" when the box is sitting near the right edge
  // actually gets you 1200, not whatever scraps of space were left.
  function setWidthPx(v) {
    const w = clamp(Math.round(v), 24, naturalSize.w);
    setCrop((c) => {
      const x = clamp(c.x, 0, naturalSize.w - w);
      return { ...c, x, w };
    });
  }
  function setHeightPx(v) {
    ...
    setCrop((c) => {
      const y = clamp(c.y, 0, naturalSize.h - h);
      return { ...c, y, h };
    });
  }
