...
5. Identify the governing law / jurisdiction the contract states for itself (a "Governing Law" clause, or similar). If one is stated, include it as its own entry in keyClauses. Your fairness judgments are general pattern-matching, not verified legal analysis for that specific jurisdiction — rules on security deposits, notice periods, tenant/employee protections, and non-compete enforceability vary significantly by country and even by state/province. If a flagged clause's legality plausibly depends on local law (e.g. deposit handling, non-compete enforceability, notice requirements), say so explicitly in that flag's reason rather than treating it as a universal standard — e.g. "this may not be enforceable in some jurisdictions" rather than a flat verdict. If no governing law is stated in the document at all, note that in the summary — an agreement with no stated jurisdiction is itself worth knowing about.
